...
ylabel('$$ \omega/N $$','FontName','Helvatica','FontSize',20,'Interpreter','latex');
xlim(ax,[0,2])
ylim(ax,[0,3.5])

le = legend(la,{sprintf('$$mH_s$$ = %0.1f',nHs(1)),...
                sprintf('$$mH_s$$ = %0.1f',nHs(2)),...
                sprintf('$$mH_s$$ = %0.1f',nHs(3))},...
...
ax = axes;
set(ax,'FontName','Helvetica','FontSize',16);
plot(1e-3*L,cg,'LineWidth',1);
le = legend({sprintf('$$mH_s$$ = %0.1f',nHs(1)),...
                sprintf('$$mH_s$$ = %0.1f',nHs(2)),...
                sprintf('$$mH_s$$ = %0.1f',nHs(3))},...
                'Location','southeast','FontSize',20,'Interpreter','latex');
grid on
box on
set(ax,'FontName','Helvetica','FontSize',16)
xlabel('Wavelength (km)','FontName','Helvetica','FontSize',20);
ylabel('Phase velocity (m/s)','FontName','Helvetica','FontSize',20);
xlim([0,700])

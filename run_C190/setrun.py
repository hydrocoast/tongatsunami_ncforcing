...
#------------------------------
    ...
    if clawdata.output_style==1:
        ...
        clawdata.output_times.extend([i*600.0 for i in range(0,43)]) # every 10 min, 0 to 7 h
    ...
    regions.append([1, 2, clawdata.t0, clawdata.tfinal, 110.0, 160.0,  0.0, 40.0])
    regions.append([1, 3, clawdata.t0, clawdata.tfinal, 115.0, 150.0, 10.0, 35.0])
    regions.append([1, 4, clawdata.t0, clawdata.tfinal, 120.0, 140.0, 15.0, 33.0])

    ## Level 5
    topo_file = topotools.Topography(os.path.join(topodir, topoflist['Amami']), topo_type=3)
    regions.append([1, 5, 2.0*3600.0, clawdata.tfinal, topo_file.x[0], topo_file.x[-1], topo_file.y[0], topo_file.y[-1]])
    ...
    ## 海嶺通過前後の地点
    gauges.append([51,  141.631, 17.821, 0., 1.e10]) #
    gauges.append([101, 135.912, 21.555, 0., 1.e10]) #
    gauges.append([102, 132.947, 23.628, 0., 1.e10]) #
    gauges.append([103, 130.099, 25.571, 0., 1.e10]) #
    gauges.append([201, 133.890, 24.481, 0., 1.e10]) #
    gauges.append([202, 132.243, 25.641, 0., 1.e10]) #
    gauges.append([203, 130.643, 26.726, 0., 1.e10]) #
    ...
    fgout.nout = int((fgout.tend - fgout.tstart)/3600.0) * 10 + 1
    ...
    fgout.tstart = 3600.0*2.0
    fgout.tend = 3600.0*7.0

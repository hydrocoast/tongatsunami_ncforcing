...
if !isempty(gauges)
    ...
    for g in gauges
        if g.id > 10000
	    # DART
            local plt = plotsgaugewaveform(g; title=g.label, label=false, ylims=(-0.1,0.1), xlims=(0.0,7.0))
        else
            local plt = plotsgaugewaveform(g; title=g.label, label=false, ylims=(-0.2,0.2), xlims=(0.0,7.0))
        end

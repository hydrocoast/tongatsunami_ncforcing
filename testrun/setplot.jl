...
## set t0 time
t0_datetime = DateTime(2020,9,2,11,0,0) #UTC!
...
## plot eta
print("plotting eta ...     ")
plts = plotsamr(amrall; clims=(-0.5,0.5), c=:bwr, colorbar=true)
...
if !isempty(gauges)
    ...
    for g in gauges
        local plt = plotsgaugewaveform(g; title=g.label)
...
if !isempty(fg)
    ...
    for k = 1:nfg
        ...
        savefig(plt, joinpath(plotdir,"fgmax_"*@sprintf("%03d",k)*".png"))
        local plt = plotsfgmax(fg[k], fgmax[k], :tD; c=cgrad(:phase, 12, categorical = true))
        savefig(plt, joinpath(plotdir,"fgmax_maxtime_"*@sprintf("%03d",k)*".png"))
        ## arrival
        local plt = plotsfgmax(fg[k], fgmax[k], :tarrival; c=cgrad(:phase, 12, categorical = true))
        savefig(plt, joinpath(plotdir,"fgmax_arrivaltime_"*@sprintf("%03d",k)*".png"))
        ## maxvel
        if fg[k].nval > 1
            local plt = plotsfgmax(fg[k], fgmax[k], :v; clims=(-1e-5,1.5), c=cgrad(:jet, 15, categorical = true))
            savefig(plt, joinpath(plotdir,"fgmax_vel_"*@sprintf("%03d",k)*".png"))
            local plt = plotsfgmax(fg[k], fgmax[k], :tv; c=cgrad(:phase, 12, categorical = true))
            savefig(plt, joinpath(plotdir,"fgmax_velmaxtime_"*@sprintf("%03d",k)*".png"))
        end
    end
    print("end\n")
end
